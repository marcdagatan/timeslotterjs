...
describe("TimeSlotterJS Scheduler", () => {
  ...
  const createTestOptions = (
    ...
    start: baseDate,
      ...
      baseDate.getTime() +
        duration * (unit === IncrementUnit.Hours ? 3600000 : unit === IncrementUnit.Minutes ? 60000 : 1000)
    ),
    increment,
    incrementUnit: unit,
    bookings,
  ...
  test("allows multiple bookings per slot when maxBookings is greater than 1", () => {
    ...
    const schedule = createSchedule(createTestOptions(1, IncrementUnit.Hours, 2, bookings, 2));
